...
### Pruebas con Cucumber (BDD) - Recomendado
...
npm test
```
Ejecuta todos los tests en `tests/` (E2E e integración).

**Ejecutar pruebas E2E:**
```bash
npm run test:e2e
```
Ejecuta solo los tests E2E del flujo de compra.

**Ejecutar pruebas de integración:**
```bash
npm run test:integration
```
Ejecuta solo los tests de integración con la API de Pokémon.

**Ejecutar con interfaz gráfica:**
```bash
npm run test:ui
```
Abre la UI interactiva de Playwright para ejecutar y depurar tests.

**Ejecutar en modo debug:**
```bash
npm run test:debug
```
Ejecuta los tests en modo debug con herramientas de depuración.

**Ejecutar con navegador visible:**
```bash
npm run test:headed
```
Ejecuta los tests con el navegador visible (no headless).

### Pruebas con Cucumber (BDD)

**Ejecutar todas las pruebas con Cucumber:**
```bash
npm run test:cucumber
```
Ejecuta todos los features (E2E e integración) con salida formateada usando `@cucumber/pretty-formatter`. **Este es el método recomendado para ver logs amigables en consola.**

**Ejecutar solo pruebas E2E con Cucumber:**
```bash
npm run test:cucumber:e2e
```
Ejecuta solo el feature E2E del flujo de compra con salida formateada.

**Ejecutar solo pruebas de integración con Cucumber:**
```bash
npm run test:cucumber:integration
```
Ejecuta solo el feature de integración con la API de Pokémon con salida formateada.

### Pruebas con Playwright (Estilo tradicional)
...
- Implementación completa del patrón Page Object Model
- Separación de lógica de negocio y selectores
- Reutilización de código entre pruebas
- Métodos descriptivos y bien documentados

### Manejo de Errores
- Mensajes de error descriptivos en todos los métodos
- Validaciones explícitas con `expect()` de Playwright
- Manejo robusto de excepciones
- Logging detallado para debugging

### Documentación
- Comentarios JSDoc en todas las clases y métodos públicos
...
- World object para compartir contexto entre steps
- Salida formateada con pretty-formatter

### Validaciones Explícitas
- Todas las validaciones usan `expect()` de Playwright
- Validación de códigos de estado HTTP 200
- Validación de datos capturados vs datos en UI
- Validación de ordenamiento alfabético
- Validación de formatos (precios, URLs, etc.)
